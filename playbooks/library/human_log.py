...
class CallbackModule(CallbackBase):
    ...
    def _write_to_log(self, message):
        header_href = "<h3 id=\"{0}\"><a href=\"#{0}\">{1}</a></h3>"
        with open(self.log_path, 'a+') as f:
            f.write(header_href.format(self.timestamp, self.task))
    ...
    def v2_playbook_on_task_start(self, task, is_conditional):
        ...
        self.timestamp = now.strftime('%Y-%m-%d %H:%M:%S.%f')[:-3]
    ...
    def v2_playbook_on_play_start(self, play):
        ...
        if not self.printed_playbook:
            ...
            now = datetime.datetime.now()
            file_suffix = now.strftime('%m-%d_%H:%M:%S')
            friendly_now = now.strftime('%Y-%m-%d %H:%M:%S %Z')

            log_filename = "{0}_log_{1}.html".format(playbook_name, file_suffix)
